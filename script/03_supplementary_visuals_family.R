...
file <- "Activity_Stress_data_N46.csv"
df_multimodel <- read.csv(paste0(data_dir, file), stringsAsFactors = FALSE)


df_T29 = df_multimodel %>%
  filter(Participant == "T028") 
...
activity_classification_plots_4CH <- function(P_ID = "T001", isSave = F, file_suffix = "5_min") {
  ...
  if (!dir.exists("../figures/Activity_ML4_Visualization_5min")) {
    dir.create("../figures/Activity_ML4_Visualization_5min")
  }

  if (isSave) {
    full_path <- paste0("../figures/Activity_ML4_Visualization_5min/", P_ID, "_4CH_Activity_Classification.pdf")
    # full_path
...
#### Save the plots for Activity_ML4 Labeling - activity_classification_plots_4CH
...
# pdf("Activity_ML4_Visualization_5min/Activity_Visualization_5min.pdf", width = 20, height = 9)
...
viz_Stress_Label <- function(P_ID = "T001", isSave = FALSE) {
  ...
  complete_grid <- expand_grid(
    Participant = sprintf("T%03d", 1:48),
    Day = c("CD", "WD", "ND")
  )
...
# Save for paper
viz_Stress_Label("T030", isSave = TRUE)
viz_Stress_Label("T021", isSave = TRUE)
...
# P_list = sprintf("T%03d", c(1:12, 14:30, 32:48)) # T013 and T031 dropped
...
for (Participant in P_list) {
  print(Participant)
  combine_plots <- cowplot::plot_grid(
    activity_classification_plots_4CH(P_ID = Participant, isSave = FALSE),
    NULL,
